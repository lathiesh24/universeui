...
const CurvedLineUp = () => {
  ...
  const fixedAngles = [
    Math.PI, // 180° (Mirror of 180°)
    (5 * Math.PI) / 4, // 225° (Mirror of 135°)
    (3 * Math.PI) / 2,
  ];
  ...
  return (
    <div
      className="relative min-h-screen bg-gray-100 flex justify-end items-start select-none"
      ...
      <div className="absolute">
        <div className="relative w-60">
          ...
          {fixedAngles.map((angle, index) => {
            ...
            const x = centerX + radius * Math.sin(newAngle);
            const y = centerY - radius * Math.cos(newAngle);

            return (
              <div
                key={index}
                className={`absolute transition-all duration-500 ease-in-out`}
                style={{ left: `${x}px`, top: `0 px` }}
              >
